...
void CreateChildProcess(void);		// create Child process -- CMD.EXE (that will execute our commands written in the pipe)
DWORD WINAPI WriteToPipe(LPVOID lpParam);	// in an infinite loop (in a separate thread of execution) Parent process writes
											// commands into Child's STDIN
void ReadFromPipe(void);	// in an infinite loop parent process reads from Child's STDOUT
							// (output of the command execution in Child)
							// and print this content in Parent's console (parent's STDOUT)
void ErrorExit(PTSTR);		// error handling
